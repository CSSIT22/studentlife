                ...
                {boxType == "request" ? (
                    <Flex mt={3} gap={2} direction="column" justify={"center"} align="center" width={"100%"}>
                        {data.communityMembers
                            .filter((member: any) => member.userRole == "request")
                            .map((member: any) => {
                                return (
                    ...
                    <Flex mt={3} gap={2} direction="column" justify={"center"} align="center" width={"100%"}>
                        {data.communityMembers
                            .filter((member: any) => member.userRole == boxType)
                            .map((member: any) => {
                                return <UserList key={member.ID} userProfile={member.profile} userName={member.userName} userRole={member.userRole} />
                            })}
                    </Flex>
                )}
            </Box>
